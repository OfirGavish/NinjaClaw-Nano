/**
 * Container Runner for NinjaClaw
      ...
      JSON.stringify(
        {
          env: {
            ...
            // Model is configured via COPILOT_MODEL env var
          },
        },
        null,
        2,
...
async function buildContainerArgs(
  ...
  const onecliApplied = await onecli.applyContainerConfig(args, {
    addHostMapping: false, // NinjaClaw already handles host gateway
  ...
  if (onecliApplied) {
    ...
    // Fallback: pass GITHUB_TOKEN directly when OneCLI is not available.
    // This allows the Copilot SDK inside the container to authenticate.
    const ghToken = process.env.GITHUB_TOKEN || envSecrets.GITHUB_TOKEN;
    if (ghToken) {
      args.push('-e', `GITHUB_TOKEN=${ghToken}`);
      args.push('-e', `COPILOT_MODEL=${process.env.COPILOT_MODEL || envSecrets.COPILOT_MODEL || 'Copilot-sonnet-4.6'}`);
      logger.info({ containerName }, 'Fallback: injected GITHUB_TOKEN directly');
    } else {
      logger.error({ containerName }, 'No GITHUB_TOKEN available — agent will not be able to authenticate');
    }
  }
...
export async function runContainerAgent(
  ...
  const containerName = `NinjaClaw-${safeName}-${Date.now()}`;
