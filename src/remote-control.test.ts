...
vi.mock('./config.js', () => ({
  DATA_DIR: '/tmp/NinjaClaw-rc-test',
}));
...
describe('remote-control', () => {
  ...
  describe('startRemoteControl', () => {
    it('spawns Copilot remote-control and returns the URL', async () => {
      const proc = createMockProcess();
      spawnMock.mockReturnValue(proc);

      // Simulate URL appearing in stdout file on first poll
      stdoutFileContent =
        'Session URL: https://Copilot.ai/code?bridge=env_abc123\n';
      ...
      expect(result).toEqual({
        ok: true,
        url: 'https://Copilot.ai/code?bridge=env_abc123',
      });
      expect(spawnMock).toHaveBeenCalledWith(
        'Copilot',
        ['remote-control', '--name', 'NinjaClaw Remote'],
    ...
    it('uses file descriptors for stdout/stderr (not pipes)', async () => {
      const proc = createMockProcess();
      spawnMock.mockReturnValue(proc);
      stdoutFileContent = 'https://Copilot.ai/code?bridge=env_test\n';
      vi.spyOn(process, 'kill').mockImplementation((() => true) as any);
    ...
    it('saves state to disk after capturing URL', async () => {
      ...
      spawnMock.mockReturnValue(proc);
      stdoutFileContent = 'https://Copilot.ai/code?bridge=env_save\n';
      vi.spyOn(process, 'kill').mockImplementation((() => true) as any);
    ...
    it('returns existing URL if session is already active', async () => {
      const proc = createMockProcess();
      spawnMock.mockReturnValue(proc);
      stdoutFileContent = 'https://Copilot.ai/code?bridge=env_existing\n';
      vi.spyOn(process, 'kill').mockImplementation((() => true) as any);
      ...
      expect(result).toEqual({
        ok: true,
        url: 'https://Copilot.ai/code?bridge=env_existing',
      });
    ...
    it('starts new session if existing process is dead', async () => {
      ...
      stdoutFileContent = 'https://Copilot.ai/code?bridge=env_first\n';
      await startRemoteControl('user1', 'tg:123', '/project');
      ...
      stdoutFileContent = 'https://Copilot.ai/code?bridge=env_second\n';
      const result = await startRemoteControl('user1', 'tg:123', '/project');

      expect(result).toEqual({
        ok: true,
        url: 'https://Copilot.ai/code?bridge=env_second',
      });
  ...
  describe('stopRemoteControl', () => {
    it('kills the process and clears state', async () => {
      ...
      spawnMock.mockReturnValue(proc);
      stdoutFileContent = 'https://Copilot.ai/code?bridge=env_stop\n';
      const killSpy = vi
        .spyOn(process, 'kill')
        .mockImplementation((() => true) as any);
  ...
  describe('restoreRemoteControl', () => {
    it('restores session if state file exists and process is alive', () => {
      const session = {
        pid: 77777,
        url: 'https://Copilot.ai/code?bridge=env_restored',
        startedBy: 'user1',
      ...
      expect(active!.url).toBe('https://Copilot.ai/code?bridge=env_restored');
    });

    it('clears state if process is dead', () => {
      const session = {
        pid: 88888,
        url: 'https://Copilot.ai/code?bridge=env_dead',
        startedBy: 'user1',
        startedInChat: 'tg:123',
        startedAt: '2026-01-01T00:00:00.000Z',
      };
