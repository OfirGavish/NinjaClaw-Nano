...
```yaml
- uses: OfirGavish/NinjaClaw-Nano/repo-tokens@v1
  with:
    include: 'src/**/*.ts'
    exclude: 'src/**/*.test.ts'
```
...
The badge color reflects what percentage of an LLMs context window the codebase fills (context window size is configurable, defaults to 200k which is the size of Copilot Opus). Green for under 30%, yellow-green for 30%-50%, yellow for 50%-70%, red for 70%+.
...
| Repo | Badge |
|------|-------|
| [NinjaClaw](https://github.com/OfirGavish/NinjaClaw-Nano) | ![tokens](https://raw.githubusercontent.com/OfirGavish/NinjaClaw-Nano/main/repo-tokens/badge.svg) |
