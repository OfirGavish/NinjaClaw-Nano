...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    try {
      ...
      if (output.includes('com.NinjaClaw')) {
        // Check if it has a PID (actually running)
        const line = output.split('\n').find((l) => l.includes('com.NinjaClaw'));
        if (line) {
    ...
    try {
      execSync(`${prefix} is-active NinjaClaw`, { stdio: 'ignore' });
      service = 'running';
    } catch {
      try {
  ...
  if (fs.existsSync(envFile)) {
    ...
    if (/^(GITHUB_TOKEN|GITHUB_TOKEN|ONECLI_URL)=/m.test(envContent)) {
  ...
  if (
    fs.existsSync(
      path.join(homeDir, '.config', 'NinjaClaw', 'mount-allowlist.json'),
    )
  ) {
