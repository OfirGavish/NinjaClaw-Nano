...
export async function run(args: string[]): Promise<void> {
  ...
  // Create COPILOT.md in the new group folder from template if it doesn't exist.
  // The agent runs with CWD=/workspace/group and loads COPILOT.md from there.
  // Never overwrite an existing COPILOT.md — users customize these extensively
  ...
  const groupCopilotMdPath = path.join(
    projectRoot,
    'groups',
    parsed.folder,
    'COPILOT.md',
  );
  if (!fs.existsSync(groupCopilotMdPath)) {
    const templatePath = parsed.isMain
      ? path.join(projectRoot, 'groups', 'main', 'COPILOT.md')
      : path.join(projectRoot, 'groups', 'global', 'COPILOT.md');
    if (fs.existsSync(templatePath)) {
      fs.copyFileSync(templatePath, groupCopilotMdPath);
      logger.info(
        { file: groupCopilotMdPath, template: templatePath },
        'Created COPILOT.md from template',
      );
    }
  }

  // Update assistant name in COPILOT.md files if different from default
  ...
  if (parsed.assistantName !== 'Andy') {
    ...
    for (const mdFile of mdFiles) {
      if (fs.existsSync(mdFile)) {
        ...
        logger.info({ file: mdFile }, 'Updated COPILOT.md');
      }
    }
