...
<dict>
    ...
    <string>${projectRoot}/logs/NinjaClaw.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/NinjaClaw.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.NinjaClaw');
  } catch {
...
/**
 * Kill any orphaned NinjaClaw node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned NinjaClaw processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/NinjaClaw.service';
    ...
    unitPath = path.join(unitDir, 'NinjaClaw.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=NinjaClaw Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/NinjaClaw.log
StandardError=append:${projectRoot}/logs/NinjaClaw.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned NinjaClaw processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable NinjaClaw`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start NinjaClaw`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active NinjaClaw`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-NinjaClaw.sh');
  const pidFile = path.join(projectRoot, 'NinjaClaw.pid');

  const lines = [
    '#!/bin/bash',
    '# start-NinjaClaw.sh — Start NinjaClaw without systemd',
    ...
    '    echo "Stopping existing NinjaClaw (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting NinjaClaw..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/NinjaClaw.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/NinjaClaw.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "NinjaClaw started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/NinjaClaw.log"`,
  ];
