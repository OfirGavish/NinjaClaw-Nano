 ...
 * apostrophe in names, .env updates, COPILOT.md template copy.
 */
...
describe('file templating', () => {
  it('replaces assistant name in COPILOT.md content', () => {
...
describe('COPILOT.md template copy', () => {
  let tmpDir: string;
  ...
  function simulateRegister(
    ...
    const dest = path.join(folderDir, 'COPILOT.md');
    if (!fs.existsSync(dest)) {
      const templatePath = isMain
        ? path.join(groupsDir, 'main', 'COPILOT.md')
        : path.join(groupsDir, 'global', 'COPILOT.md');
    ...
    if (assistantName !== 'Andy') {
      const mdFiles = fs
        ...
        .map((d) => path.join(groupsDir, d, 'COPILOT.md'))
  ...
  function readGroupMd(folder: string): string {
    return fs.readFileSync(
      path.join(groupsDir, folder, 'COPILOT.md'),
      'utf-8',
    );
  }

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'NinjaClaw-register-test-'));
  ...
  it('never overwrites existing COPILOT.md on re-registration', () => {
    simulateRegister('slack_main', true);
    // User customizes the file extensively (persona, workspace, rules)
    const mdPath = path.join(groupsDir, 'slack_main', 'COPILOT.md');
    fs.writeFileSync(
      mdPath,
  ...
  it('never overwrites when non-main becomes main (isMain changes)', () => {
    ...
    const mdPath = path.join(groupsDir, 'whatsapp_casa', 'COPILOT.md');
    fs.writeFileSync(
      mdPath,
      '# Casa\n\nFamily group with PARA system, task management, shopping lists.',
    );
    // Later, user promotes to main (no trigger required) — COPILOT.md must be preserved
  ...
  it('preserves custom COPILOT.md across channels when changing main', () => {
    // Real-world scenario: WhatsApp main + customized Discord research channel
    simulateRegister('whatsapp_main', true);
    simulateRegister('discord_main', false);
    const discordPath = path.join(groupsDir, 'discord_main', 'COPILOT.md');
    fs.writeFileSync(
      discordPath,
  ...
  it('handles missing templates gracefully', () => {
    fs.unlinkSync(path.join(groupsDir, 'global', 'COPILOT.md'));
    fs.unlinkSync(path.join(groupsDir, 'main', 'COPILOT.md'));

    simulateRegister('discord_general', false);

    expect(
      fs.existsSync(path.join(groupsDir, 'discord_general', 'COPILOT.md')),
    ).toBe(false);
  });
});
