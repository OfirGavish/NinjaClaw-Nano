...
1. **Check for existing work.** Search open PRs and issues before starting:
   ```bash
   gh pr list --repo OfirGavish/NinjaClaw-Nano --search "<your feature>"
   gh issue list --repo OfirGavish/NinjaClaw-Nano --search "<your feature>"
   ```
...
NinjaClaw uses [GitHub Copilot skills](https://code.Copilot.com/docs/en/skills) — markdown files with optional supporting files that teach Copilot how to do something. There are four types of skills in NinjaClaw, each serving a different purpose.
...
Add capabilities to NinjaClaw by merging a git branch. The SKILL.md contains setup instructions; the actual code lives on a `skill/*` branch.
...
1. Fork `OfirGavish/NinjaClaw-Nano` and branch from `main`
...
- Use `${COPILOT_SKILL_DIR}` to reference files in the skill directory
...
**Examples:** `/setup`, `/debug`, `/customize`, `/update-NinjaClaw`, `/update-skills`
...
**Key difference:** These are NOT invoked by the user on the host. They're loaded by GitHub Copilot inside the container and influence how the agent behaves.
...
All skills use the [GitHub Copilot skills standard](https://code.Copilot.com/docs/en/skills):
...
- See the [skills standard](https://code.Copilot.com/docs/en/skills) for all available frontmatter fields
