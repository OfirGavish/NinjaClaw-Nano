...
<p align="center">
  NinjaClaw —— 您的专属 Copilot 助手，在容器中安全运行。它轻巧易懂，并能根据您的个人需求灵活定制。
</p>
...
通过 GitHub Copilot，NinjaClaw 可以动态重写自身代码，根据您的需求定制功能。
...
NinjaClaw 用一个您能快速理解的代码库，为您提供了同样的核心功能。只有一个进程，少数几个文件。智能体（Agent）运行在具有文件系统隔离的真实 Linux 容器中，而不是依赖于权限检查。
...
然后运行 `/setup`。GitHub Copilot 会处理一切：依赖安装、身份验证、容器设置、服务配置。

> **注意：** 以 `/` 开头的命令（如 `/setup`、`/add-whatsapp`）是 [GitHub Copilot 技能](https://code.Copilot.com/docs/en/skills)。请在 `copilot` CLI 提示符中输入，而非在普通终端中。

## 设计哲学

**小巧易懂：** 单一进程，少量源文件。无微服务、无消息队列、无复杂抽象层。让 GitHub Copilot 引导您轻松上手。
...
**为单一用户打造:** 这不是一个框架，是一个完全符合您个人需求的、可工作的软件。您可以 Fork 本项目，然后让 GitHub Copilot 根据您的精确需求进行修改和适配。

**定制即代码修改:** 没有繁杂的配置文件。想要不同的行为？直接修改代码。代码库足够小，这样做是安全的。

**AI 原生:** 无安装向导（由 GitHub Copilot 指导安装）。无需监控仪表盘，直接询问 Copilot 即可了解系统状况。无调试工具（描述问题，Copilot 会修复它）。

**技能（Skills）优于功能（Features）:** 贡献者不应该向代码库添加新功能（例如支持 Telegram）。相反，他们应该贡献像 `/add-telegram` 这样的 [GitHub Copilot 技能](https://code.Copilot.com/docs/en/skills)，这些技能可以改造您的 fork。最终，您得到的是只做您需要事情的整洁代码。

**最好的工具套件，最好的模型:** 本项目运行在 GitHub Copilot SDK 之上，这意味着您直接运行的就是 GitHub Copilot。GitHub Copilot 高度强大，其编码和问题解决能力使其能够修改和扩展 NinjaClaw，为每个用户量身定制。

## 功能支持

- **多渠道消息** - 通过 WhatsApp、Telegram、Discord、Slack 或 Gmail 与您的助手对话。使用 `/add-whatsapp` 或 `/add-telegram` 等技能添加渠道，可同时运行一个或多个。
- **隔离的群组上下文** - 每个群组都拥有独立的 `COPILOT.md` 记忆和隔离的文件系统。它们在各自的容器沙箱中运行，且仅挂载所需的文件系统。
...
没有需要学习的配置文件。直接告诉 GitHub Copilot 您想要什么：
...
如果您想添加 Telegram 支持，不要创建一个 PR 同时添加 Telegram 和 WhatsApp。而是贡献一个技能文件 (`.copilot/skills/add-telegram/SKILL.md`)，教 GitHub Copilot 如何改造一个 NinjaClaw 安装以使用 Telegram。
...
可以。NinjaClaw 支持任何 API 兼容的模型端点。在 `.env` 文件中设置以下环境变量：
...
- 兼容 github-copilot API 格式的自定义模型部署

注意：为获得最佳兼容性，模型需支持 github-copilot API 格式。

**我该如何调试问题？**

问 GitHub Copilot。"为什么计划任务没有运行？" "最近的日志里有什么？" "为什么这条消息没有得到回应？" 这就是 AI 原生的方法。
