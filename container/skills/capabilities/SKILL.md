---
name: capabilities
description: Show what this NinjaClaw instance can do — installed skills, available tools, and system info. Read-only. Use when the user asks what the bot can do, what's installed, or runs /capabilities.
---

# /capabilities — System Capabilities Report

Generate a structured read-only report of what this NinjaClaw instance can do.
...
- **MCP:** mcp__NinjaClaw__* (messaging, tasks, group management)

### 3. MCP server tools

The NinjaClaw MCP server exposes these tools (via `mcp__NinjaClaw__*` prefix):
...
```bash
ls /workspace/group/COPILOT.md 2>/dev/null && echo "Group memory: yes" || echo "Group memory: no"
