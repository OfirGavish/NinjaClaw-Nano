 ...
 * Note: Pre-compact hooks are a GitHub Copilot SDK feature.
 * With Copilot SDK, this is kept as a no-op placeholder for future use.
 */
function createPreCompactHook(assistantName?: string): any {
  return async (input: any, _toolUseId: any, _context: any) => {
    const preCompact = input as { transcript_path?: string; session_id?: string };
...
/**
 * Run a single query using the GitHub Copilot SDK.
 * Replaces the GitHub Copilot SDK's query() with Copilot SDK's session-based model.
 * Maintains the same IPC contract (MessageStream, _close sentinel, writeOutput).
 */
async function runQuery(
  ...
  // Load global COPILOT.md as additional system context (shared across all groups)
  const globalCopilotMdPath = '/workspace/global/COPILOT.md';
  let globalCopilotMd: string | undefined;
  if (!containerInput.isMain && fs.existsSync(globalCopilotMdPath)) {
    globalCopilotMd = fs.readFileSync(globalCopilotMdPath, 'utf-8');
  }

  // Initialize Copilot SDK client
  const copilotModel = process.env.COPILOT_MODEL || 'Copilot-sonnet-4.6';
  ...
  try {
    ...
    const sessionConfig: Parameters<typeof client.createSession>[0] = {
      sessionId: sessionId || `ninjaclaw-${containerInput.groupFolder}-${Date.now()}`,
      model: copilotModel,
      onPermissionRequest: approveAll,
      systemMessage: globalCopilotMd
        ? { content: globalCopilotMd }
        : undefined,
      infiniteSessions: {
        enabled: true,
    ...
    session.on('assistant.message', (event: any) => {
      resultCount++;
      const text = event.data?.content ?? null;
      log(`Result #${resultCount}: ${text ? `text=${text.slice(0, 200)}` : 'empty'}`);
      writeOutput({
        status: 'success',
        result: text,
        newSessionId,
    ...
    // After initial response, poll for IPC follow-up messages
    const pollIpcDuringQuery = () => {
      if (!ipcPolling) return;
      if (shouldClose()) {
        log('Close sentinel detected, ending');
      ...
      for (const text of messages) {
        log(`Piping IPC message into session (${text.length} chars)`);
        session.send({ prompt: text }).catch((err) => {
          log(`Failed to send IPC message: ${err instanceof Error ? err.message : String(err)}`);
        });
      }
    ...
    // Disconnect session (preserves data on disk)
  ...
  return { newSessionId, lastAssistantUuid: undefined, closedDuringQuery };
}
...
async function main(): Promise<void> {
  ...
  // Credentials: GitHub token is passed via GITHUB_TOKEN env var.
  // The Copilot SDK uses it for authentication.
