...
│  │    • mcp__NinjaClaw__* (scheduler tools via IPC)                │    │
│  │                                                                │    │
│  └──────────────────────────────────────────────────────────────┘    │
│                                                                       │
└───────────────────────────────────────────────────────────────────────┘
...
| Agent | @github/Copilot-agent-sdk (0.2.29) | Run Copilot with tools and MCP servers |
...
│
├── container/
│   ├── Dockerfile                 # Container image (runs as 'node' user, includes GitHub Copilot CLI)
...
│   ├── sessions/                  # Per-group session data (.copilot/ dirs with JSONL transcripts)
│   ├── env/env                    # Copy of .env for container mounting
│   └── ipc/                       # Container IPC (messages/, tasks/)
│
├── logs/                          # Runtime logs (gitignored)
│   ├── NinjaClaw.log               # Host stdout
│   └── NinjaClaw.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.NinjaClaw.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'NinjaClaw-agent:latest';
...
**Option 1: Copilot subscription (OAuth token)**
```bash
GITHUB_TOKEN=ghp_...
```
The token can be extracted from `~/.copilot/.credentials.json` if you're logged in to GitHub Copilot.

**Option 2: Pay-per-use API Key**
```bash
GITHUB_TOKEN=sk-ant-api03-...
```

Only the authentication variables (`GITHUB_TOKEN` and `GITHUB_TOKEN`) are extracted from `.env` and written to `data/env/env`, then mounted into the container at `/workspace/env-dir/env` and sourced by the entrypoint script. This ensures other environment variables in `.env` are not exposed to the agent. This workaround is needed because some container runtimes lose `-e` environment variables when using `-i` (interactive mode with piped stdin).
...
- `{{PROJECT_ROOT}}` - Absolute path to your NinjaClaw installation
...
NinjaClaw uses a hierarchical memory system based on COPILOT.md files.

### Memory Hierarchy

| Level | Location | Read By | Written By | Purpose |
|-------|----------|---------|------------|---------|
| **Global** | `groups/COPILOT.md` | All groups | Main only | Preferences, facts, context shared across all conversations |
| **Group** | `groups/{name}/COPILOT.md` | That group | That group | Group-specific context, conversation memory |
...
1. **Agent Context Loading**
   ...
   - GitHub Copilot SDK with `settingSources: ['project']` automatically loads:
     - `../COPILOT.md` (parent directory = global memory)
     - `./COPILOT.md` (current directory = group memory)

2. **Writing Memory**
   - When user says "remember this", agent writes to `./COPILOT.md`
   - When user says "remember this globally" (main channel only), agent writes to `../COPILOT.md`
...
Sessions enable conversation continuity - Copilot remembers what you talked about.
...
4. Session transcripts are stored as JSONL files in `data/sessions/{group}/.copilot/`
...
7. Router invokes GitHub Copilot SDK:
   ...
   └── mcpServers: NinjaClaw (scheduler)
   │
   ▼
8. Copilot Processes message:
   ├── Reads COPILOT.md files for context
...
- `@Andy what's the weather?` → ✅ Triggers Copilot
...
NinjaClaw has a built-in scheduler that runs tasks as full agents in their group's context.
...
Copilot: [calls mcp__NinjaClaw__schedule_task]
        {
...
### NinjaClaw MCP (built-in)

The `NinjaClaw` MCP server is created dynamically per agent call with the current group's context.
...
NinjaClaw runs as a single macOS launchd service.

### Startup Sequence

When NinjaClaw starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned NinjaClaw containers from previous runs
...
### Service: com.NinjaClaw

**launchd/com.NinjaClaw.plist:**
```xml
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/NinjaClaw.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/NinjaClaw.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.NinjaClaw.plist ~/Library/LaunchAgents/

# Start service
launchctl load ~/Library/LaunchAgents/com.NinjaClaw.plist

# Stop service
launchctl unload ~/Library/LaunchAgents/com.NinjaClaw.plist

# Check status
launchctl list | grep NinjaClaw

# View logs
tail -f logs/NinjaClaw.log
```
...
WhatsApp messages could contain malicious instructions attempting to manipulate Copilot's behavior.
...
- Copilot's built-in safety training
...
|------------|------------------|-------|
| Copilot CLI Auth | data/sessions/{group}/.copilot/ | Per-group isolation, mounted to /home/node/.copilot/ |
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep NinjaClaw` |
| "GitHub Copilot process exited with code 1" | Container runtime failed to start | Check logs; NinjaClaw auto-starts container runtime but may fail |
| "GitHub Copilot process exited with code 1" | Session mount path wrong | Ensure mount is to `/home/node/.copilot/` not `/root/.copilot/` |
| Session not continuing | Session ID not saved | Check SQLite: `sqlite3 store/messages.db "SELECT * FROM sessions"` |
| Session not continuing | Mount path mismatch | Container user is `node` with HOME=/home/node; sessions must be at `/home/node/.copilot/` |
| "QR code expired" | WhatsApp session expired | Delete store/auth/ and restart |
| "No groups registered" | Haven't added groups | Use `@Andy add group "Name"` in main |

### Log Location

- `logs/NinjaClaw.log` - stdout
- `logs/NinjaClaw.error.log` - stderr
