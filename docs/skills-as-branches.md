...
This document covers **feature skills** — skills that add capabilities via git branch merges. This is the most complex skill type and the primary way NinjaClaw is extended.

NinjaClaw has four types of skills overall. See [CONTRIBUTING.md](../CONTRIBUTING.md) for the full taxonomy:
...
This replaces the previous `skills-engine/` system (three-way file merging, `.NinjaClaw/` state, manifest files, replay, backup/restore) with plain git operations and Copilot for conflict resolution.

## How It Works

### Repository structure

The upstream repo (`OfirGavish/NinjaClaw-Nano`) maintains:

- `main` — core NinjaClaw (no skill code)
...
- `/setup`, `/debug`, `/update-NinjaClaw`, `/customize`, `/update-skills`
...
- Live in the marketplace repo (`OfirGavish/NinjaClaw-Nano-skills`)
...
Copilot plugin install NinjaClaw-skills@NinjaClaw-skills --scope project
```
...
NinjaClaw's `.copilot/settings.json` registers the official marketplace:
...
```json
{
  "extraKnownMarketplaces": {
    "NinjaClaw-skills": {
      "source": {
        "source": "github",
        "repo": "OfirGavish/NinjaClaw-Nano-skills"
      }
    }
  }
}
...
The marketplace repo uses GitHub Copilot's plugin structure:

```
OfirGavish/NinjaClaw-Nano-skills/
  .Copilot-plugin/
    marketplace.json              # Plugin catalog
  plugins/
    NinjaClaw-skills/              # Single plugin bundling all official skills
      .Copilot-plugin/
...
Multiple skills are bundled in one plugin — installing `NinjaClaw-skills` makes all feature skills available at once. Individual skills don't need separate installation.
...
- Built into `/update-NinjaClaw` — after merging main, optionally check for skill updates
...
1. Fork `OfirGavish/NinjaClaw-Nano` on GitHub (click the Fork button)
2. Clone your fork:
   ```bash
   git clone https://github.com/<you>/NinjaClaw.git
   cd NinjaClaw
   ```
3. Run GitHub Copilot:
   ```bash
   Copilot
   ```
...
Users who previously ran `git clone https://github.com/OfirGavish/NinjaClaw-Nano.git` and have local customizations:

1. Fork `OfirGavish/NinjaClaw-Nano` on GitHub
2. Reroute remotes:
   ```bash
   git remote rename origin upstream
   git remote add origin https://github.com/<you>/NinjaClaw.git
...
3. From this point, `origin` = their fork, `upstream` = OfirGavish/NinjaClaw-Nano
...
In both cases:
- Delete the `.NinjaClaw/` directory (no longer needed)
...
Run `/add-discord` in GitHub Copilot (discovered via the marketplace plugin), or manually:
...
This is the same as the existing `/update-NinjaClaw` skill's merge path.

### Updating skills

Run `/update-skills` or let `/update-NinjaClaw` check after a core update. For each previously-merged skill branch that has new commits, Copilot offers to merge the updates.
...
# Create PR from my-fix to OfirGavish/NinjaClaw-Nano:main
```
...
1. Fork `OfirGavish/NinjaClaw-Nano`
2. Branch from `main`
...
4. Add the skill's SKILL.md to the marketplace repo (`OfirGavish/NinjaClaw-Nano-skills`)
...
1. Maintains a fork of NinjaClaw (e.g., `alice/NinjaClaw`)
2. Creates `skill/*` branches on their fork with their custom skills
3. Creates a marketplace repo (e.g., `alice/NinjaClaw-skills`) with a `.Copilot-plugin/marketplace.json` and plugin structure

### Adding a community marketplace

If the community contributor is trusted, they can open a PR to add their marketplace to NinjaClaw's `.copilot/settings.json`:
...
```json
{
  "extraKnownMarketplaces": {
    ...
    "alice-NinjaClaw-skills": {
      "source": {
        "source": "github",
        "repo": "alice/NinjaClaw-skills"
      }
    }
  }
}
...
Once merged, all NinjaClaw users automatically discover the community marketplace alongside the official one.
...
```bash
Copilot plugin install alice-skills@alice-NinjaClaw-skills --scope project
```
...
- **Community skills use the same merge pattern.** The SKILL.md just points to a different remote:
  ```bash
  git remote add alice https://github.com/alice/NinjaClaw.git
  git fetch alice skill/my-cool-feature
  git merge alice/skill/my-cool-feature
  ```
- **Users can also add marketplaces manually.** Even without being listed in settings.json, users can run `/plugin marketplace add alice/NinjaClaw-skills` to discover skills from any source.
...
A flavor is a curated fork of NinjaClaw — a combination of skills, custom changes, and configuration tailored for a specific use case (e.g., "NinjaClaw for Sales," "NinjaClaw Minimal," "NinjaClaw for Developers").
...
AskUserQuestion: "Start with a flavor or default NinjaClaw?"
- Default NinjaClaw
- NinjaClaw for Sales — Gmail + Slack + CRM (maintained by alice)
- NinjaClaw Minimal — Telegram-only, lightweight (maintained by bob)

If a flavor is chosen:

```bash
git remote add <flavor-name> https://github.com/alice/NinjaClaw.git
...
- `upstream` — `OfirGavish/NinjaClaw-Nano` (core updates)
...
```yaml
flavors:
  - name: NinjaClaw for Sales
    repo: alice/NinjaClaw
    description: Gmail + Slack + CRM integration, daily pipeline summaries
    maintainer: alice

  - name: NinjaClaw Minimal
    repo: bob/NinjaClaw
...
- **GitHub topics** — flavor forks can tag themselves with `NinjaClaw-flavor` for searchability
...
- `.NinjaClaw/` state directory

Operational skills (`setup`, `debug`, `update-NinjaClaw`, `customize`, `update-skills`) remain on main in `.copilot/skills/`.
...
```
1. Fork OfirGavish/NinjaClaw-Nano on GitHub
2. git clone https://github.com/<you>/NinjaClaw.git
3. cd NinjaClaw
4. Copilot
5. /setup
```
...
- Check if `upstream` remote exists; if not, add it: `git remote add upstream https://github.com/OfirGavish/NinjaClaw-Nano.git`
- Check if `origin` points to the user's fork (not OfirGavish). If it points to OfirGavish, guide them through the fork migration.
- **Install marketplace plugin:** `Copilot plugin install NinjaClaw-skills@NinjaClaw-skills --scope project` — makes all feature skills available (hot-loaded, no restart)
...
The `.copilot/skills/` directory on `main` retains only operational skills (setup, debug, update-NinjaClaw, customize, update-skills). Feature skills (add-discord, add-telegram, etc.) live in the marketplace repo, installed via `Copilot plugin install` during `/setup` or `/customize`.
...
- **Marketplace repo** (`OfirGavish/NinjaClaw-Nano-skills`) — single GitHub Copilot plugin bundling SKILL.md files for all feature skills
...
>
> We've simplified how skills work in NinjaClaw. Instead of a custom skills engine, skills are now git branches that you merge in.
>
...
> - No more `.NinjaClaw/` state directory or skills engine
>
...
>
> **If you previously applied skills via the old system**, your code changes are already in your working tree — nothing to redo. You can delete the `.NinjaClaw/` directory. Future skills and updates use the branch-based approach.
>
> **Discovering skills:** Skills are now available through GitHub Copilot's plugin marketplace. Run `/plugin` in GitHub Copilot to browse and install available skills.
...
>
> **Want to run your own skill marketplace?** Maintain skill branches on your fork and create a marketplace repo. Open a PR to add it to NinjaClaw's auto-discovered marketplaces — or users can add it manually via `/plugin marketplace add`.
