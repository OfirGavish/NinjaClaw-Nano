# Running NinjaClaw in Docker Sandboxes (Manual Setup)

This guide walks through setting up NinjaClaw inside a [Docker Sandbox](https://docs.docker.com/ai/sandboxes/) from scratch — no install script, no pre-built fork. You'll clone the upstream repo, apply the necessary patches, and have agents running in full hypervisor-level isolation.
...
The sandbox provides a MITM proxy at `host.docker.internal:3128` that handles network access and injects your github-copilot API key automatically.
...
- **github-copilot API key** (the sandbox proxy manages injection)
...
Enter the sandbox:
```bash
docker sandbox run shell-NinjaClaw-workspace
```
...
## Step 3: Clone and Install NinjaClaw

NinjaClaw must live inside the workspace directory — Docker-in-Docker can only bind-mount from the shared workspace path.
...
WORKSPACE=/Users/you/NinjaClaw-workspace

# Move into workspace so DinD mounts work
mv NinjaClaw "$WORKSPACE/NinjaClaw"
cd "$WORKSPACE/NinjaClaw"
...
NinjaClaw needs several patches to work inside a Docker Sandbox. These handle proxy routing, CA certificates, and Docker-in-Docker mount restrictions.
...
In `src/container-runtime.ts`, the `cleanupOrphans()` function matches containers by the `NinjaClaw-` prefix. Inside a sandbox, the sandbox container itself may match (e.g., `NinjaClaw-docker-sandbox`). Filter out the current hostname:
...
npx tsx scripts/apply-skill.ts .copilot/skills/add-telegram
...
npx tsx scripts/apply-skill.ts .copilot/skills/add-whatsapp
...
You don't need to set `GITHUB_TOKEN` manually. The sandbox proxy intercepts requests and replaces `proxy-managed` with your real key automatically.
...
```bash
cd ~ && git clone https://github.com/OfirGavish/NinjaClaw-Nano.git && mv NinjaClaw /path/to/workspace/NinjaClaw
```
