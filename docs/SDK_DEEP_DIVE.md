...
Findings from reverse-engineering `@github/Copilot-agent-sdk` v0.2.29–0.2.34 to understand how `query()` works, why agent teams subagents were being killed, and how to fix it. Supplemented with official SDK reference docs.
...
Agent Runner (our code)
  └── query() → SDK (sdk.mjs)
        └── spawns CLI subprocess (cli.js)
              └── Copilot API calls, tool execution
...
2. **Call the github-copilot API** (via `mW1` streaming function)
...
| `additionalDirectories` | `string[]` | `[]` | Additional directories Copilot can access |
...
| `model` | `string` | Default from CLI | Copilot model to use |
| `outputFormat` | `{ type: 'json_schema', schema: JSONSchema }` | `undefined` | Structured output format |
| `pathToCopilotExecutable` | `string` | Uses built-in | Path to GitHub Copilot executable |
...
| `settingSources` | `SettingSource[]` | `[]` (none) | Which filesystem settings to load. Must include `'project'` to load COPILOT.md |
| `stderr` | `(data: string) => void` | `undefined` | Callback for stderr output |
| `systemPrompt` | `string \| { type: 'preset'; preset: 'copilot'; append?: string }` | `undefined` | System prompt. Use preset to get GitHub Copilot's prompt, with optional `append` |
| `tools` | `string[] \| { type: 'preset'; preset: 'copilot' }` | `undefined` | Tool configuration |
...
// 'project' → .copilot/settings.json (version controlled)
// 'local'   → .copilot/settings.local.json (gitignored)
```
...
type SDKAssistantMessage = {
  ...
  uuid: UUID;
  session_id: string;
  message: APIAssistantMessage; // From github-copilot SDK
...
Copilot responded with text only — it decided it has completed the task. The API's `stop_reason` will be `"end_turn"`. The SDK does NOT make this decision — it's entirely driven by Copilot's model output.
...
2. assistant/user       → Copilot reasoning, tool calls, tool results
...
| `mW1` | github-copilot API streaming caller |
