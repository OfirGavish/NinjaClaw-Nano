...
**`OfirGavish/NinjaClaw-Nano`** (upstream) — core engine with skill definitions (`.copilot/skills/`). No channel code on `main`.

**Channel forks** (`NinjaClaw-whatsapp`, `NinjaClaw-telegram`, `NinjaClaw-slack`, etc.) — each fork = upstream + one channel's code applied. Users clone upstream, then merge a fork into their clone to add a channel.
...
user clones upstream main
  ├── merges NinjaClaw-whatsapp fork  → adds WhatsApp
...
```bash
# In your local NinjaClaw checkout
git checkout main && git pull

# For a fork:
git fetch NinjaClaw-whatsapp
git checkout -B whatsapp-merge NinjaClaw-whatsapp/main
git merge main
...
git push NinjaClaw-whatsapp HEAD:main
...
After any main change that touches shared files (`package.json`, `src/index.ts`, `COPILOT.md`, etc.). Small frequent merges = trivial conflicts. Large infrequent merges = painful.

## Fork setup

When creating a new channel fork:

1. Fork `NinjaClaw` to `NinjaClaw-{channel}`
