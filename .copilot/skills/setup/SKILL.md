---
name: setup
description: Run initial NinjaClaw setup. Use when user wants to install dependencies, authenticate messaging channels, register their main channel, or start the background services. Triggers on "setup", "install", "configure NinjaClaw", or first-time setup requests.
---
...
**Case A — `origin` points to `OfirGavish/NinjaClaw-Nano` (user cloned directly):**

The user cloned instead of forking. AskUserQuestion: "You cloned NinjaClaw directly. We recommend forking so you can push your customizations. Would you like to set up a fork?"
- Fork now (recommended) — walk them through it
- Continue without fork — they'll only have local changes

If fork: instruct the user to fork `OfirGavish/NinjaClaw-Nano` on GitHub (they need to do this in their browser), then ask them for their GitHub username. Run:
```bash
git remote rename origin upstream
git remote add origin https://github.com/<their-username>/NinjaClaw.git
...
**Case C — both `origin` (user's fork) and `upstream` (OfirGavish) exist:**

Already configured. Continue.

**Verify:** `git remote -v` should show `origin` → user's repo, `upstream` → `OfirGavish/NinjaClaw-Nano.git`.
...
2. **Fresh start** — "Skip migration and set up NinjaClaw from scratch."
...
If an github-copilot secret is listed, confirm with user: keep or reconfigure? If keeping, skip to step 5.
...
> Run `copilot setup-token` in another terminal. It will output a token — copy it but don't paste it here.
...
1. **Dashboard** — description: "Best if you have a browser on this machine. Open ${ONECLI_URL} and add the secret in the UI. Use type 'github-copilot' and paste your token as the value."
2. **CLI** — description: "Best for remote/headless servers. Run: `onecli secrets create --name github-copilot --type github-copilot --value YOUR_TOKEN --host-pattern api.github.com`"
...
2. **CLI** — description: "Best for remote/headless servers. Run: `onecli secrets create --name github-copilot --type github-copilot --value YOUR_KEY --host-pattern api.github.com`"
...
**After user confirms:** verify with `onecli secrets list` that an github-copilot secret exists. If not, ask again.
...
1. **Copilot subscription (Pro/Max)** — description: "Uses your existing Copilot Pro or Max subscription. Run `copilot setup-token` in another terminal to get your token."
2. **github-copilot API key** — description: "Pay-per-use API key from console.github-copilot.com."

For subscription: tell the user to run `copilot setup-token` in another terminal. Stop and wait for the user to confirm they have completed this step successfully before proceeding.
...
```bash
echo 'GITHUB_TOKEN=<their-key>' >> .env
```
...
- macOS: `launchctl unload ~/Library/LaunchAgents/com.NinjaClaw.plist`
- Linux: `systemctl --user stop NinjaClaw` (or `systemctl stop NinjaClaw` if root)

Run `npx tsx setup/index.ts --step service` and parse the status block.

**If FALLBACK=wsl_no_systemd:** WSL without systemd detected. Tell user they can either enable systemd in WSL (`echo -e "[boot]\nsystemd=true" | sudo tee /etc/wsl.conf` then restart WSL) or use the generated `start-NinjaClaw.sh` wrapper.
...
- macOS: check `launchctl list | grep NinjaClaw`. If PID=`-` and status non-zero, read `logs/NinjaClaw.error.log`.
- Linux: check `systemctl --user status NinjaClaw`.
...
- SERVICE=stopped → `npm run build`, then restart: `launchctl kickstart -k gui/$(id -u)/com.NinjaClaw` (macOS) or `systemctl --user restart NinjaClaw` (Linux) or `bash start-NinjaClaw.sh` (WSL nohup)
...
Tell user to test: send a message in their registered chat. Show: `tail -f logs/NinjaClaw.log`

## Troubleshooting

**Service not starting:** Check `logs/NinjaClaw.error.log`. Common: wrong Node path (re-run step 7), credential system not running (Docker: check `curl ${ONECLI_URL}/api/health`; Apple Container: check `.env` credentials), missing channel credentials (re-invoke channel skill).

**Container agent fails ("GitHub Copilot process exited with code 1"):** Ensure the container runtime is running — `open -a Docker` (macOS Docker), `container system start` (Apple Container), or `sudo systemctl start docker` (Linux). Check container logs in `groups/main/logs/container-*.log`.

**No response to messages:** Check trigger pattern. Main channel doesn't need prefix. Check DB: `npx tsx setup/index.ts --step verify`. Check `logs/NinjaClaw.log`.

**Channel not connecting:** Verify the channel's credentials are set in `.env`. Channels auto-enable when their credentials are present. For WhatsApp: check `store/auth/creds.json` exists. For token-based channels: check token values in `.env`. Restart the service after any `.env` change.

**Unload service:** macOS: `launchctl unload ~/Library/LaunchAgents/com.NinjaClaw.plist` | Linux: `systemctl --user stop NinjaClaw`


## 9. Diagnostics

1. Use the Read tool to read `.copilot/skills/setup/diagnostics.md`.
