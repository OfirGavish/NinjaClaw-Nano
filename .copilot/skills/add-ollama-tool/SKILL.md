...
This skill adds a stdio-based MCP server that exposes local Ollama models as tools for the container agent. Copilot remains the orchestrator but can offload work to local models, and can optionally manage the model library directly.
...
```bash
tail -f logs/NinjaClaw.log | grep -i ollama
```
