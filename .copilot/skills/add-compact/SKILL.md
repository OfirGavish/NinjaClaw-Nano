...
> **Note:** `upstream` is the remote pointing to `OfirGavish/NinjaClaw-Nano`. If using a different remote name, substitute accordingly.
...
1. Start NinjaClaw in dev mode: `npm run dev`
...
```bash
git clone <your-fork> /tmp/NinjaClaw-test
cd /tmp/NinjaClaw-test
...
- **No config file.** NinjaClaw's philosophy is customization through code changes, not configuration sprawl.
