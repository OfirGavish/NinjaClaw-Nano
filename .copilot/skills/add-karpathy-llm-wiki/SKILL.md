...
description: Add a persistent wiki knowledge base to a NinjaClaw group. Based on Karpathy's LLM Wiki pattern. Triggers on "add wiki", "wiki", "knowledge base", "llm wiki", "karpathy wiki".
---

# Add Karpathy LLM Wiki

Set up a persistent wiki knowledge base on NinjaClaw, based on Karpathy's LLM Wiki pattern.

## Step 1: Read the pattern

Read `${COPILOT_SKILL_DIR}/llm-wiki.md` — this is the full LLM Wiki idea as written by Karpathy. Understand it thoroughly before proceeding. Summarize the core idea to the user briefly, then discuss what they want to build.
...
Edit the group's COPILOT.md to add a wiki section. This is critical — it's what turns the agent into a wiki maintainer. It should:
...
Copilot has built-in `WebFetch`, but it returns a summary, not the full document. For wiki ingestion of a URL where the full text matters, the container skill and COPILOT.md should instruct Copilot to use bash commands to download full files instead. For example:

```bash
curl -sLo sources/filename.pdf "<url>"
```

If the document is a webpage, then Copilot can use fetch or `agent-browser` to open the page and extract full text if available. The container skill and COPILOT.md should note this so Copilot gets full content for sources rather than summaries.
...
If yes, create a NinjaClaw scheduled task that runs in the wiki group. This is NOT a GitHub Copilot cron job — it's a NinjaClaw group task that runs in the agent container. Insert it into the SQLite database:
