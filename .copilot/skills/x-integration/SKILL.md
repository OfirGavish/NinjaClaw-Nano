---
name: x-integration
description: X (Twitter) integration for NinjaClaw. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** NinjaClaw v1.0.0. Directory structure may change in future versions.
...
1. **NinjaClaw is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep NinjaClaw (macOS) or systemctl --user status NinjaClaw (Linux)
```
...
| `NinjaClaw_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/NinjaClaw.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into NinjaClaw, make the following modifications:
...
```typescript
// @ts-ignore - Copied during Docker build from .copilot/skills/x-integration/
...
Change build context from `container/` to project root (required to access `.copilot/skills/`):
```bash
# Find:
...
First, update the build context paths (required to access `.copilot/skills/` from project root):
```dockerfile
# Find:
...
COPY .copilot/skills/x-integration/agent.ts ./src/skills/x-integration/
```

## Setup

All paths below are relative to project root (`NinjaClaw_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep NinjaClaw  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status NinjaClaw
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/NinjaClaw.log | tail -20

# Script errors
grep -i "error\|failed" logs/NinjaClaw.log | tail -20
```
...
docker run NinjaClaw-agent ls -la /app/src/skills/
```
