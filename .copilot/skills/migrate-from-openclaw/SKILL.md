...
description: Migrate from OpenClaw to NinjaClaw. Detects existing OpenClaw installation, extracts identity, channel credentials, scheduled tasks, and other config, then guides interactive migration. Triggers on "migrate from openclaw", "openclaw migration", "import from openclaw".
---

# Migrate from OpenClaw

Guide the user through migrating their OpenClaw installation to NinjaClaw. This is a conversation, not a batch job. Read OpenClaw state, discuss it with the user, make judgment calls together about what to bring over and how.

**Principle:** Never silently copy data. Read it, explain it, discuss where it belongs in NinjaClaw's architecture, show proposed changes before applying. Credentials must be masked when displayed (first 4 + `...` + last 4 characters). Make judgment calls about what's core vs. reference material.
...
- **Settings Migrated** — timezone, github-copilot_credential (masked), sender_allowlist (created/skipped)
- **Identity & Memory** — paths of files created, which COPILOT.md was edited
...
```bash
npx tsx ${COPILOT_SKILL_DIR}/scripts/discover-openclaw.ts
```
...
- Channels: list each, note which NinjaClaw supports (whatsapp, telegram, slack, discord) and which it doesn't
...
- **Container isolation:** NinjaClaw runs each agent in an isolated Linux container (Docker or Apple Container). OpenClaw runs everything in one process. This means stronger isolation but also means each group is its own sandbox.
- **Group-based memory:** In OpenClaw, all groups under one agent share the same SOUL.md, MEMORY.md, and IDENTITY.md. In NinjaClaw, each group has its own filesystem and COPILOT.md. Shared state goes in `groups/global/COPILOT.md` (mounted read-only into all non-main containers).
- **Channel skills:** In OpenClaw, channels are configured in `openclaw.json`. In NinjaClaw, channels are installed as code via skills (`/add-telegram`, `/add-whatsapp`, etc.) and configured through `.env` variables.
- **Simpler config:** NinjaClaw has no config file — behavior is in the code and `COPILOT.md` files. Credentials live in `.env` or the OneCLI vault.
...
**NinjaClaw model:** Each group is a completely separate agent running in its own Linux container. Separate filesystem, separate memory, separate COPILOT.md. The bot in your family chat and your work chat are different agents that don't know about each other — unless you explicitly share state via `groups/global/COPILOT.md`, which is mounted read-only into all non-main containers.
...
AskUserQuestion: "In OpenClaw, your groups shared the same personality and memory. In NinjaClaw, each group is a fully separate agent. How would you like to handle this?"

1. **Shared personality (recommended if your groups had the same bot)** — "I'll put the shared personality, identity, and user context in `groups/global/COPILOT.md`. Every group sees it. Each group can add its own customizations on top."
...
Before registering groups, confirm the assistant name — it's used for trigger patterns and COPILOT.md templates.

IDENTITY_NAME from discovery gives the OpenClaw name. Ask the user: "Your OpenClaw assistant was named `<IDENTITY_NAME>`. Want to keep this name in NinjaClaw?" If they want a different name, ask what it should be. If IDENTITY_NAME was empty, ask them to choose a name (default: "Andy").

The register step's `--assistant-name` flag writes `ASSISTANT_NAME` to `.env` and updates COPILOT.md templates automatically — no manual `.env` write needed.

### Registering groups

The discovery script provides detected groups in the GROUPS field (format: `channel:id(name)=>NinjaClaw_jid`). These are extracted from OpenClaw's session store and channel config.

For each group the user wants to bring over, pre-register it:

```bash
npx tsx setup/index.ts --step register -- --jid "<NinjaClaw_jid>" --name "<group_name>" --folder "<channel>_<slug>" --trigger "@<confirmed_name>" --channel <channel> --assistant-name "<confirmed_name>"
```

Only pass `--assistant-name` on the first registration (it updates all COPILOT.md templates globally).
...
**Important:** Registration requires the database to exist. If the environment step hasn't been run yet, run it first: `npx tsx setup/index.ts --step environment`. Registration also creates the group folder under `groups/` and copies the COPILOT.md template.

Register groups from all channels — including channels NinjaClaw doesn't yet support (signal, matrix, etc.). The registration stores the JID and metadata in the database, ready for when that channel is added later. Groups won't receive messages until their channel code is installed, but the registration, group folder, and COPILOT.md will be ready.

## Phase 2: Settings from Config

Before identity/memory, extract settings from `openclaw.json` that map directly to NinjaClaw setup. Read the config file with the Read tool (`<STATE_DIR>/openclaw.json` or `clawdbot.json`).

### Timezone

Check `agents.defaults.userTimezone` in the config. If present and it's a valid IANA timezone (e.g. `America/New_York`, `Asia/Jerusalem`), write it to `.env` as `TZ=<timezone>`. NinjaClaw's setup step 2a reads `TZ` from `.env` (`src/config.ts:84-97`) and will skip the autodetection prompt.

### github-copilot Credentials

Check for github-copilot API keys or tokens in OpenClaw's auth system. OpenClaw stores credentials in `<STATE_DIR>/auth-profiles.json` or `<STATE_DIR>/agents/main/agent/auth-profiles.json` with this structure:
...
Profile IDs follow `provider:identifier` format. Look for any profile where `provider` is `"github-copilot"`. The credential field depends on the `type`:
...
Also check:
1. `<STATE_DIR>/.env` — for `GITHUB_TOKEN` or `GITHUB_TOKEN`
2. Config `models.providers` — for github-copilot provider entries with `apiKey`

If found, offer to save to `.env`. This pre-fills the NinjaClaw setup credential step (step 4) so the user doesn't need to re-enter it. Use the same masking approach — show first 4 + last 4 characters, write the full value directly.
...
NinjaClaw uses `~/.config/NinjaClaw/sender-allowlist.json` with this format:
```json
{
...
If OpenClaw had allowlists configured, show the user what was set and offer to create the NinjaClaw equivalent. Map:
...
- **Shared personality:** Core identity goes in `groups/global/COPILOT.md` (seen by all groups). Group-specific customizations go in each group's own COPILOT.md.
...
The assistant name was already confirmed and written to `.env` in Phase 1. Here, focus on the rest of the identity — create an `identity.md` file with the full identity details (emoji, creature, vibe, personality traits, etc.). If shared personality was chosen in Phase 1, put it alongside `groups/global/COPILOT.md`. Otherwise, put it in `groups/main/`.

### SOUL.md

Read `<STATE_DIR>/workspace/SOUL.md` if it exists. Then read `groups/main/COPILOT.md`.

COPILOT.md is always loaded into the agent's context — it's the agent's continuous instructions. Not everything from SOUL.md needs to be there. Discuss with the user what belongs where:

- **In COPILOT.md (always loaded):** Core personality traits, communication style, key behavioral rules. Weave these into the existing COPILOT.md structure — adjust the opening description under the `# <Name>` heading, modify the tone in the Communication section.
...
- Shared personality → edit `groups/global/COPILOT.md` for the core traits, create `groups/global/soul.md` for the extended content. All groups will see both.
- Separate / main only → edit `groups/main/COPILOT.md`, create `groups/main/soul.md`.

Add a reference in the relevant COPILOT.md: "Your personality and extended behavioral guidelines are in `soul.md`. Refer to it for identity questions or when crafting responses that need your full character."
...
Create `groups/main/user-context.md` with the user information. Add a reference in COPILOT.md: "Information about your user is in `user-context.md`. Read it when you need context about who you're talking to."

Ask if they want any critical user facts (name, timezone, key preferences) directly in COPILOT.md for always-on awareness.

### MEMORY.md

Read `<STATE_DIR>/workspace/MEMORY.md` if it exists.

Show the contents and discuss what's worth keeping. Some memory entries may be stale or OpenClaw-specific. Create `groups/main/memories.md` for relevant items. Add a reference in COPILOT.md.
...
1. **Copy as-is (recommended for many files)** — "I'll create a `daily-memories/` folder in your group directory and copy them over. Your agent can reference them when needed."
   ...
   - Add a reference in COPILOT.md: "Historical daily memory files from your previous system are in `daily-memories/`. Refer to them when you need context about past events or observations."
...
For each detected plugin, present the name to the user and discuss whether to set it up in NinjaClaw. Read the OpenClaw config section to understand what it is, then:

1. **If NinjaClaw has a matching skill** — check the available NinjaClaw skills list for an equivalent (e.g. `/add-voice-transcription` for whisper). If found, save the API key to `.env` and invoke that skill.
...
3. **If the OpenClaw plugin was a CLI tool** — read the config to identify the exact tool. If it's an npm package, add it to the container's Dockerfile. Add a note to the group's COPILOT.md that the tool is available and how to invoke it.
...
- TOOLS.md: NinjaClaw agents have their own tool discovery; this doesn't transfer
- HEARTBEAT.md: NinjaClaw uses scheduled tasks instead
- BOOTSTRAP.md: NinjaClaw uses COPILOT.md and container skills instead
- AGENTS.md: Already covered in the Phase 1 groups discussion

## Phase 4: Channel Credentials

For each channel found in the discovery results, handle it based on NinjaClaw support:

### Supported channels (whatsapp, telegram, slack, discord)

Run the credential extraction script with `--write-env .env` so it writes credentials directly to NinjaClaw's `.env` file. The script never emits raw credential values to stdout — only masked versions.

First, run without `--write-env` to preview:

```bash
npx tsx ${COPILOT_SKILL_DIR}/scripts/extract-channel-credentials.ts --state-dir <STATE_DIR> --channel <name>
```

Parse the status block. Key fields: HAS_CREDENTIAL, CREDENTIAL_MASKED, NinjaClaw_ENV_VAR.
...
```bash
npx tsx ${COPILOT_SKILL_DIR}/scripts/extract-channel-credentials.ts --state-dir <STATE_DIR> --channel <name> --write-env .env
```

The script writes the credential directly to `.env` using the correct NinjaClaw variable name (e.g. `TELEGRAM_BOT_TOKEN`). Check the status block for `WRITTEN_TO` and `WRITTEN_COUNT` to confirm.
...
For Slack: there are two credentials (bot token + app token). The script handles both in one run — check `HAS_CREDENTIAL_2` and `NinjaClaw_ENV_VAR_2` in the status block.
...
Explain briefly: "NinjaClaw doesn't have a `<channel>` integration yet, but channels are added over time via skills. Any groups from this channel were already registered in Phase 1 — they'll activate when the channel is added."
...
If jobs exist, read `${COPILOT_SKILL_DIR}/MIGRATE_CRONS.md` for the full OpenClaw cron format, NinjaClaw table schema, field mapping, and SQL insert template. Follow those instructions for each job.
...
If MCP_SERVERS was non-empty in discovery, these can be ported. GitHub Copilot supports MCP servers natively. Read the OpenClaw config's `mcp.servers` section to get each server's details (`command`, `args`, `env`, `url`).

MCP servers in NinjaClaw are registered in the agent-runner source code. Before editing, grep for `mcpServers` in `container/agent-runner/src/` to find the current location — it's expected to be in `index.ts` in the `query()` options, but may have moved. For each OpenClaw MCP server the user wants to bring over:
...
- These don't map directly but NinjaClaw can be customized to support them
...
- **Exec approvals / command allowlist:** NinjaClaw uses container isolation instead — the agent runs with `--dangerously-skip-permissions` inside a sandboxed container
- **Human delay:** Not applicable in NinjaClaw's container model
- **Compaction:** Handled by GitHub Copilot SDK automatically
- **TTS:** Not built into NinjaClaw
- **Model configuration:** NinjaClaw uses whatever github-copilot model the credential provides access to
...
**Migrated:**
- Assistant name → `.env` ASSISTANT_NAME + COPILOT.md templates updated
- Groups → registered in database, folders created with COPILOT.md templates
- Timezone → `.env` TZ
- github-copilot credential → `.env` (for setup to pick up)
- Sender allowlists → `~/.config/NinjaClaw/sender-allowlist.json`
- Personality → COPILOT.md (core) + `soul.md` (extended), placed per Phase 1 decision (global or per-group)
...
Remind: "Run `/setup` next to complete your NinjaClaw installation. Channel credentials are already prepared in `.env`. When setup asks which channels to enable, select the ones we configured."
...
**Multi-agent complexity:** If the user had many agents with different configs, focus on the primary/default agent first. Additional agents can be set up as separate NinjaClaw groups later.
