 ...
 *
 * Usage: npx tsx .copilot/skills/migrate-from-openclaw/scripts/discover-openclaw.ts [--state-dir <path>]
 *
...
function emitStatus(fields: Record<string, string | number | boolean>): void {
  const lines = ['=== NinjaClaw MIGRATE: DISCOVERY ==='];
...
/**
 * Map OpenClaw session key channel:kind:id to NinjaClaw JID format.
 * OpenClaw keys: "whatsapp:group:120...@g.us", "telegram:group:-10012345"
 * NinjaClaw JIDs: "120...@g.us", "tg:-10012345", "dc:12345", "slack:C12345"
 */
function toNinjaClawJid(channel: string, id: string): string {
  switch (channel) {
    case 'whatsapp':
...
function main(): void {
  ...
  // Format groups as "channel:id(name)" — also include NinjaClaw JID mapping
  const groupList = groups
    .map(
      (g) =>
        `${g.channel}:${g.id}(${g.name})=>${toNinjaClawJid(g.channel, g.id)}`,
    )
    .join('|');
  ...
  // Dump raw top-level config keys so Copilot can see what exists
