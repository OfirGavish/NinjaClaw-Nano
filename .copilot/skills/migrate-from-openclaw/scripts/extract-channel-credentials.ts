 ...
 * directly to the NinjaClaw .env file.
 *
 * Usage: npx tsx .copilot/skills/migrate-from-openclaw/scripts/extract-channel-credentials.ts \
...
function emitStatus(fields: Record<string, string | number | boolean>): void {
  const lines = ['=== NinjaClaw MIGRATE: CREDENTIAL ==='];
...
interface ChannelCredentialSpec {
  ...
  fields: string[];
  // Corresponding NinjaClaw env var names
  envVars: string[];
}
...
function main(): void {
  ...
  // using useMultiFileAuthState (same as NinjaClaw). The files are directly compatible.
  if (channel === 'whatsapp') {
    const authPaths = [
  ...
  if (!spec) {
    emitStatus({
      CHANNEL: channel,
      HAS_CREDENTIAL: false,
      NOTE: `Channel "${channel}" is not supported by NinjaClaw. Supported: telegram, discord, slack, whatsapp.`,
    });
    return;
  }
  ...
  const fields: Record<string, string | number | boolean> = {
    ...
    NinjaClaw_ENV_VAR: primary.envVar,
  };
  ...
  if (results.length > 1) {
    for (let i = 1; i < results.length; i++) {
      ...
      fields[`NinjaClaw_ENV_VAR${suffix}`] = extra.envVar;
      if (extra.note) {
