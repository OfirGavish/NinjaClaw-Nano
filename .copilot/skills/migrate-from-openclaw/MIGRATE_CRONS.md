# Migrating OpenClaw Cron Jobs to NinjaClaw Scheduled Tasks
...
## NinjaClaw `scheduled_tasks` Table
...
- `delivery.mode:"webhook"` — NinjaClaw has no webhook delivery. Discuss with the user: this could be implemented as a task `script` that runs `curl` to hit the webhook endpoint.
- `failureAlert` — NinjaClaw has no failure alert system. Note this to the user.
- `wakeMode` — NinjaClaw tasks always wake the agent immediately.
- `payload.model`, `payload.thinking`, `payload.timeoutSeconds` — NinjaClaw doesn't support per-task model/thinking config. These are handled by the SDK.
- `deleteAfterRun` — NinjaClaw `"once"` tasks are marked `"completed"` after running, not deleted.
