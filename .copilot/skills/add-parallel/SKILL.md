...
Adds Parallel AI MCP integration to NinjaClaw for advanced web research capabilities.
...
- **Non-blocking Design** - Uses NinjaClaw scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. NinjaClaw already set up and running
...
Find the line:
```typescript
const allowedVars = ['GITHUB_TOKEN', 'GITHUB_TOKEN'];
```

Replace with:
```typescript
const allowedVars = ['GITHUB_TOKEN', 'GITHUB_TOKEN', 'PARALLEL_API_KEY'];
```
...
Add Parallel AI MCP servers after the NinjaClaw server:
```typescript
const mcpServers: Record<string, any> = {
  NinjaClaw: ipcMcp
};
...
### 5. Add Usage Instructions to COPILOT.md

Add Parallel AI usage instructions to `groups/main/COPILOT.md`:
...
3. Create a polling scheduled task using `mcp__NinjaClaw__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__NinjaClaw__send_message
   4. Use mcp__NinjaClaw__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo NinjaClaw-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep NinjaClaw  # macOS
...
- Check task runs: `tail -f logs/NinjaClaw.log | grep "scheduled task"`
...
3. Remove Web Research Tools section from groups/main/COPILOT.md
4. Rebuild: `./container/build.sh && npm run build`
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.NinjaClaw` (macOS) or `systemctl --user restart NinjaClaw` (Linux)
