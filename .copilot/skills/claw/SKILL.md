...
- Python 3.8 or later
- NinjaClaw installed with a built and tagged container image (`NinjaClaw-agent:latest`)
- Either `container` (Apple Container, macOS 15+) or `docker` available in `PATH`

## Install

Run this skill from within the NinjaClaw directory. The script auto-detects its location, so the symlink always points to the right place.
...
### Override the NinjaClaw directory
