...
description: Add a macOS menu bar status indicator for NinjaClaw. Shows a bolt icon with a green/red dot indicating whether NinjaClaw is running, with Start, Stop, and Restart controls. macOS only.
---

# Add macOS Menu Bar Status Indicator

Adds a persistent menu bar icon that shows NinjaClaw's running status and lets the user
...
```bash
launchctl list | grep com.NinjaClaw.statusbar
```
...
```bash
mkdir -p dist
swiftc -O -o dist/statusbar "${COPILOT_SKILL_DIR}/add/src/statusbar.swift"
```
...
Create `~/Library/LaunchAgents/com.NinjaClaw.statusbar.plist`, substituting the actual values
...
<dict>
    <key>Label</key>
    <string>com.NinjaClaw.statusbar</string>
...
```bash
launchctl load ~/Library/LaunchAgents/com.NinjaClaw.statusbar.plist
```
...
> The bolt icon should now appear in your macOS menu bar. Click it to see NinjaClaw's status and control the service.
>
> - **Green dot** — NinjaClaw is running
> - **Red dot** — NinjaClaw is stopped
>
...
```bash
launchctl unload ~/Library/LaunchAgents/com.NinjaClaw.statusbar.plist
rm ~/Library/LaunchAgents/com.NinjaClaw.statusbar.plist
rm dist/statusbar
```
