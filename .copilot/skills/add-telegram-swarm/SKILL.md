...
### Step 5: Update COPILOT.md Files

#### 5a. Add global message formatting rules

Read `groups/global/COPILOT.md` and add a Message Formatting section:
...
#### 5b. Update existing group COPILOT.md headings

In any group COPILOT.md that has a "WhatsApp Formatting" section (e.g. `groups/main/COPILOT.md`), rename the heading to reflect multi-channel support:
...
For each Telegram group that will use agent teams, create or update its `groups/{folder}/COPILOT.md` with these instructions. Read the existing COPILOT.md first (or `groups/global/COPILOT.md` as a base) and add the Agent Teams section:
...
1. *Share progress in the group* via `mcp__NinjaClaw__send_message` with a `sender` parameter matching their exact role/character name (e.g., `sender: "Marine Biologist"` or `sender: "Alexander Hamilton"`). This makes their messages appear from a dedicated bot in the Telegram group.
...
Also add `TELEGRAM_BOT_POOL` to the launchd plist (`~/Library/LaunchAgents/com.NinjaClaw.plist`) in the `EnvironmentVariables` dict if using launchd.
...
# Linux:
# systemctl --user restart NinjaClaw
```
...
>
> Check logs: `tail -f logs/NinjaClaw.log | grep -i pool`
...
2. Check pool initialized: `grep "Pool bot" logs/NinjaClaw.log`
...
Check the group's `COPILOT.md` has the Agent Teams instructions. The lead agent reads this when creating teammates and must include the `send_message` + `sender` instructions in each teammate's prompt.
...
6. Remove Agent Teams section from group COPILOT.md files
7. Remove `TELEGRAM_BOT_POOL` from `.env`, `data/env/env`, and launchd plist/systemd unit
8. Rebuild: `npm run build && ./container/build.sh && launchctl unload ~/Library/LaunchAgents/com.NinjaClaw.plist && launchctl load ~/Library/LaunchAgents/com.NinjaClaw.plist` (macOS) or `npm run build && ./container/build.sh && systemctl --user restart NinjaClaw` (Linux)
