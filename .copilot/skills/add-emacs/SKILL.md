---
name: add-emacs
description: Add Emacs as a channel. Opens an interactive chat buffer and org-mode integration so you can talk to NinjaClaw from within Emacs (Doom, Spacemacs, or vanilla). Uses a local HTTP bridge — no bot token or external service needed.
---

# Add Emacs Channel

This skill adds Emacs support to NinjaClaw, then walks through interactive setup.
...
- **Code review** — select a region and send it with `NinjaClaw-org-send`; the response appears as a child heading inline in your org file
...
- **Schedule tasks** — ask Andy to set a reminder or create a scheduled NinjaClaw task (e.g. "remind me tomorrow to review the PR")
...
If an `upstream` remote pointing to `https://github.com/OfirGavish/NinjaClaw-Nano.git` is missing,
add it:
...
- `emacs/NinjaClaw.el` — Emacs Lisp package (`NinjaClaw-chat`, `NinjaClaw-org-send`)
...
The `NinjaClaw.el` package requires only Emacs 27.1+ built-in libraries (`url`, `json`, `org`) — no package manager setup needed.
...
```elisp
;; NinjaClaw — personal AI assistant channel
(load (expand-file-name "~/src/NinjaClaw/emacs/NinjaClaw.el"))

(map! :leader
      :prefix ("N" . "NinjaClaw")
      :desc "Chat buffer"  "c" #'NinjaClaw-chat
      :desc "Send org"     "o" #'NinjaClaw-org-send)
```
...
```elisp
;; NinjaClaw — personal AI assistant channel
(load-file "~/src/NinjaClaw/emacs/NinjaClaw.el")

(spacemacs/set-leader-keys "aNc" #'NinjaClaw-chat)
(spacemacs/set-leader-keys "aNo" #'NinjaClaw-org-send)
```
...
(global-set-key (kbd "C-c n c") #'NinjaClaw-chat)
(global-set-key (kbd "C-c n o") #'NinjaClaw-org-send)
```
...
```elisp
(setq NinjaClaw-auth-token "<your-token>")
```

If `EMACS_CHANNEL_PORT` was changed from the default, also add:

```elisp
(setq NinjaClaw-port <your-port>)
```
...
```bash
npm run build
launchctl kickstart -k gui/$(id -u)/com.NinjaClaw  # macOS
...
Either a stale NinjaClaw process is running, or 8766 is taken by another app.
...
Or change the port in `.env` (`EMACS_CHANNEL_PORT=8767`) and update `NinjaClaw-port` in Emacs config.

### No response from agent

Check:
1. NinjaClaw is running: `launchctl list | grep NinjaClaw` (macOS) or `systemctl --user status NinjaClaw` (Linux)
2. Emacs group is registered: `sqlite3 store/messages.db "SELECT * FROM registered_groups WHERE jid = 'emacs:default'"`
3. Logs show activity: `tail -50 logs/NinjaClaw.log`

If the group is not registered, it will be created automatically on the next NinjaClaw restart.
...
```elisp
;; M-x describe-variable RET NinjaClaw-auth-token RET
```

Must exactly match `EMACS_AUTH_TOKEN` in `.env`.

### NinjaClaw.el not loading

Check the path is correct:

```bash
ls ~/src/NinjaClaw/emacs/NinjaClaw.el
```

If NinjaClaw is cloned elsewhere, update the `load`/`load-file` path in your Emacs config.
...
# Linux:
# systemctl --user stop NinjaClaw
# npm run dev
# systemctl --user start NinjaClaw
```
...
1. Delete `src/channels/emacs.ts`, `src/channels/emacs.test.ts`, and `emacs/NinjaClaw.el`
2. Remove `import './emacs.js'` from `src/channels/index.ts`
3. Remove the NinjaClaw block from your Emacs config file
...
6. Rebuild: `npm run build && launchctl kickstart -k gui/$(id -u)/com.NinjaClaw` (macOS) or `npm run build && systemctl --user restart NinjaClaw` (Linux)