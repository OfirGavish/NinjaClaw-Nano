...
This skill switches NinjaClaw's container runtime from Docker to Apple Container (macOS-only). It uses the skills engine for deterministic code changes, then walks through verification.
...
```bash
grep -q 'NinjaClaw proxy' /etc/pf.conf 2>/dev/null || echo '# NinjaClaw proxy — block LAN access to credential proxy
...
```bash
echo '{}' | container run -i --entrypoint /bin/echo NinjaClaw-agent:latest "Container OK"
```
