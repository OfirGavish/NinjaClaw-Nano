...
NinjaClaw users fork the repo and customize it — changing config values, editing source files, modifying personas, adding skills. When upstream ships updates or refactors, `git merge` produces painful conflicts because the same core files were changed on both sides.
...
The migration guide is markdown, not structured data. It needs to capture the full range of what a user might customize, with enough implementation detail that a fresh Copilot session can reapply it without having seen the original code. Standard changes (config values, simple logic) can be described briefly. Non-standard changes (specific APIs, custom integrations, unusual patterns) need code snippets and precise instructions.
...
Check remotes with `git remote -v`. If `upstream` is missing, ask for the URL (default: `https://github.com/OfirGavish/NinjaClaw-Nano.git`), add it, then `git fetch upstream --prune`.
...
Check for existing guide: `.NinjaClaw-migrations/guide.md` or `.NinjaClaw-migrations/index.md`.

**Determine the tier based on the total diff from base:**

### Tier 1: Lightweight — suggest `/update-NinjaClaw` instead
...
Tell the user the scope is small and suggest `/update-NinjaClaw` might be simpler. Let them choose.
...
For Tier 1:
- **Use /update-NinjaClaw** — simpler merge-based approach
...
> Explore this NinjaClaw fork to identify all changes from the upstream base. Run these commands and report back:
>
...
> 4. `ls .copilot/skills/` — installed skills
...
> 4. Assess detail level: could a fresh Copilot session reproduce this from intent alone, or does it need specific code snippets, API details, import paths?
...
**Storage:** `.NinjaClaw-migrations/guide.md` for Tier 2. `.NinjaClaw-migrations/` directory with `index.md` and section files for Tier 3.
...
The guide is structured markdown that a fresh Copilot session can follow to reproduce this user's exact setup on a clean upstream checkout.

Structure:

```markdown
# NinjaClaw Migration Guide
...
Custom skills (user-created, not from upstream): `.copilot/skills/my-custom-skill/` — copy as-is from main tree.
...
**Judging detail level:** For each customization, assess whether a fresh Copilot session could reproduce it from intent alone:
...
For behavior customizations (COPILOT.md files): copy from the main tree. These are user content, not code.
...
1. Stop the service (do this directly):
   ```bash
   launchctl unload ~/Library/LaunchAgents/com.NinjaClaw.plist 2>/dev/null || true
   ```
...
cp -r "$WORKTREE_PATH/.NinjaClaw-migrations" /tmp/NinjaClaw-migrations-backup 2>/dev/null || true
...
cp -r /tmp/NinjaClaw-migrations-backup/* .NinjaClaw-migrations/ 2>/dev/null || true
rm -rf /tmp/NinjaClaw-migrations-backup
```
...
1. Use the Read tool to read `.copilot/skills/migrate-NinjaClaw/diagnostics.md`.
