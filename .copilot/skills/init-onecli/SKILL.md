---
name: init-onecli
description: Install and initialize OneCLI Agent Vault. Migrates existing .env credentials to the vault. Use after /update-NinjaClaw brings in OneCLI as a breaking change, or for first-time OneCLI setup.
---

# Initialize OneCLI Agent Vault

This skill installs OneCLI, configures the Agent Vault gateway, and migrates any existing `.env` credentials into it. Run this after `/update-NinjaClaw` introduces OneCLI as a breaking change, or any time OneCLI needs to be set up from scratch.
...
If the command succeeds, OneCLI is installed, check for an github-copilot secret:

```bash
onecli secrets list
```

If an github-copilot secret exists, tell the user OneCLI is already configured and working. Use AskUserQuestion:
...
If `@onecli-sh/sdk` is NOT in package.json, the codebase hasn't been updated to use OneCLI yet. Tell the user to run `/update-NinjaClaw` first to get the OneCLI integration, then retry `/init-onecli`. Stop here.
...
|---|---|---|
| `GITHUB_TOKEN` | `github-copilot` | `api.github.com` |
| `GITHUB_TOKEN` | `github-copilot` | `api.github.com` |
| `GITHUB_TOKEN` | `github-copilot` | `api.github.com` |
...
**github-copilot API key** (`GITHUB_TOKEN=sk-ant-...`):
```bash
onecli secrets create --name github-copilot --type github-copilot --value <key> --host-pattern api.github.com
```

**Copilot OAuth token** (`GITHUB_TOKEN=...` or `GITHUB_TOKEN=...`):
```bash
onecli secrets create --name github-copilot --type github-copilot --value <token> --host-pattern api.github.com
```

After successful migration, remove the credential lines from `.env`. Use the Edit tool to remove only the credential variable lines (`GITHUB_TOKEN`, `GITHUB_TOKEN`, `GITHUB_TOKEN`). Keep all other `.env` entries intact (e.g. `ONECLI_URL`, `TELEGRAM_BOT_TOKEN`, channel tokens).
...
Tell the user: "Migrated your github-copilot credentials from `.env` to the OneCLI Agent Vault. The raw keys have been removed from `.env` — they're now managed by OneCLI and will be injected at request time without entering containers."

### Offer to migrate other container-facing credentials

After handling github-copilot credentials (whether migrated or freshly registered), scan `.env` again for remaining credential variables that containers use for outbound API calls.

**Important:** Only migrate credentials that containers use via outbound HTTPS. Channel tokens (`TELEGRAM_BOT_TOKEN`, `SLACK_BOT_TOKEN`, `SLACK_APP_TOKEN`, `DISCORD_BOT_TOKEN`) are used by the NinjaClaw host process to connect to messaging platforms — they must stay in `.env`.
...
Check if OneCLI already has an github-copilot secret:
```bash
onecli secrets list
```

If an github-copilot secret already exists, skip to Phase 4.
...
Restart the service:
- macOS (launchd): `launchctl kickstart -k gui/$(id -u)/com.NinjaClaw`
- Linux (systemd): `systemctl --user restart NinjaClaw`
...
```bash
tail -30 logs/NinjaClaw.log | grep -i "onecli\|gateway"
```
...
**Container gets no credentials:** Verify `ONECLI_URL` is set in `.env` and the gateway has an github-copilot secret (`onecli secrets list`).

**Old .env credentials still present:** This skill should have removed them. Double-check `.env` for `GITHUB_TOKEN`, `GITHUB_TOKEN`, or `GITHUB_TOKEN` and remove them manually if still present.
