---
name: add-gmail
description: Add Gmail integration to NinjaClaw. Can be configured as a tool (agent reads/sends emails when triggered from WhatsApp) or as a full channel (emails can trigger the agent, schedule tasks, and receive replies). Guides through GCP OAuth setup and implements the integration.
---
...
```bash
git remote add gmail https://github.com/OfirGavish/NinjaClaw-Nano-gmail.git
```
...
If the user chose channel mode, append the following to `groups/main/COPILOT.md` (before the formatting section):
...
>    - Application type: **Desktop app**, name: anything (e.g., "NinjaClaw Gmail")
...
Tell the user to send themselves a test email. The agent should pick it up within a minute. Monitor: `tail -f logs/NinjaClaw.log | grep -iE "(gmail|email)"`.
...
5. Rebuild: `cd container && ./build.sh && cd .. && npm run build && launchctl kickstart -k gui/$(id -u)/com.NinjaClaw` (macOS) or `systemctl --user restart NinjaClaw` (Linux)
...
8. Rebuild: `cd container && ./build.sh && cd .. && npm run build && launchctl kickstart -k gui/$(id -u)/com.NinjaClaw` (macOS) or `systemctl --user restart NinjaClaw` (Linux)
